...
def inject_custom_css() -> None:
    ...
    lobby_background = asset_css_url("dana_landing_start_1920x1080.png")
    st.html(
        f"""
        <style>
        :root {{
...
def _chart_title(figure: go.Figure) -> str:
    ...
    return str(title or "Detail Grafik")
...
@st.dialog("Detail Grafik", width="large")
...
def render_chart_card(
    ...
    with header:
        st.html(f'<div class="chart-card-title">{escape(title)}</div>')
    with action:
        if st.button(
            "⛶",
            key=f"chart_expand_{chart_id}",
            help=f"Buka detail grafik {title}",
        ):
...
def render_sidebar_nav() -> None:
    ...
    if st.sidebar.button(
        ...
        width="stretch",
        help="Filter diterapkan hanya setelah tombol Terapkan Filter ditekan.",
    ):
...
def render_lobby_page(
    ...
    st.html(
        ...
        <div class="lobby-topbar">
            {_logo_img_tag()}
            <div class="lobby-help">Butuh bantuan? Gunakan Lampiran Presentasi.</div>
        </div>
        """
    )
    st.html(
        ...
        <section class="lobby-shell fade-in">
            <div class="lobby-hero">
                ...
                <div class="lobby-visual" aria-hidden="true">
                    {asset_img_tag("dana_wallet_cluster_720x720.png", alt="")}
                </div>
            </div>
...
def render_top_header(
    ...
    with st.container(key="top_header"):
        ...
        with filter_column:
            panel_open = bool(st.session_state.get("filter_open", False))
            ...
            if st.button(
                ...
                st.session_state.filter_open = not panel_open
                st.rerun()
        with brand_column:
...
def main() -> None:
    ...
    with st.sidebar:
        if st.session_state.get("filter_open", False):
            st.html(
                '<div class="filter-rail-marker">'
                '<strong>Filter &amp; Control</strong>'
                '<span>Perubahan baru memengaruhi statistik setelah '
                'Terapkan Filter ditekan.</span></div>'
            )
            if st.button(
                "Kembali ke Navigasi",
                key="close_filter_rail",
                width="stretch",
            ):
