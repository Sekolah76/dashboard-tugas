    ...
    with st.sidebar:
        if st.session_state.get("filter_open", False):
            ...
            filter_icon_html = (
                ...
                f'style="width:40px;height:40px;object-fit:contain;border-radius:10px;margin-bottom:.4rem;" alt="" />'
                if filter_icon else ""
            )
            st.html(
                ...
                '<span>Ubah filter di bawah, lalu tekan '
                '<em>Terapkan Filter</em> untuk memperbarui data.</span></div>'
            )
            if st.button(
                "✕ Tutup Filter",
