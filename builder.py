...
def create_kpi_card(icon, label, value, caption, progress=None, p_color=C_PRIMARY):
    ...
    if progress is not None:
        ...
        <div class="kpi-progress-bg">
            <div class="kpi-progress-bar" style="width: {progress}%; background-color: {p_color};"></div>
        </div>
        '''
    return f'''
...
def render_hero(total_survey, total_ulasan, avg_skor, avg_rating, pct_pos):
    ...
    </div>'''.replace('{{total_survey}}', str(total_survey)).replace('{{total_ulasan}}', str(total_ulasan)).replace('{{avg_skor:.2f}}', f'{avg_skor:.2f}').replace('{{avg_rating:.2f}}', f'{avg_rating:.2f}').replace('{{pct_pos:.1f}}', f'{pct_pos:.1f}').replace('{{now_str}}', now_str), unsafe_allow_html=True)
...
def render_sidebar_filters(survey_df, survey_cols, review_df, review_cols, load_errors):
    with st.sidebar:
        ...
        if review_df is not None and not review_df.empty:
            f_rating = st.multiselect("Rating ⭐", options=[1, 2, 3, 4, 5], default=[1, 2, 3, 4, 5], format_func=lambda x: f"{'⭐'*x} ({x})", key="r_sel")
...
with open('data_logic.py', 'r', encoding='utf-8') as f:
    data_logic = f.read()
