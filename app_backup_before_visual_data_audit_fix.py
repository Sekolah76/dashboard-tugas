...
def inject_custom_css() -> None:
    st.html(
        ...
        .st-key-control_panel {{
            ...
            border-radius: 20px;
            background:
                radial-gradient(circle at 98% 2%, rgba(56,189,248,.12), transparent 18rem),
...
def donut_chart(
    ...
    figure = go.Figure(
        go.Pie(
            ...
            hovertemplate=(
                "<b>%{label}</b><br>"
                "%{value} dari %{customdata[0]} "
                "(%{customdata[1]:.1f}%)<br>"
                "Scope: %{customdata[2]}<extra></extra>"
            ),
        )
    )
...
def bar_chart(
    ...
    if denominator is not None:
        customdata = [
            [denominator, float(value) / denominator * 100 if denominator else 0.0, scope_label]
            for value in values
        ]
        hovertemplate = (
            "<b>%{x}</b><br>%{y} dari %{customdata[0]} "
            "(%{customdata[1]:.1f}%)<br>"
            "Scope: %{customdata[2]}<extra></extra>"
        )
    else:
        ...
        hovertemplate = (
            "<b>%{x}</b><br>Nilai: %{y}<br>"
            "Scope: %{customdata[0]}<extra></extra>"
        )
    figure = go.Figure(
        go.Bar(
            x=labels,
    ...
    figure = go.Figure(
        go.Bar(
            ...
            hovertemplate=(
                ...
                "Rata-rata: <b>%{x:.2f}</b><br>"
                "Interpretasi: %{customdata[1]}<br>"
                "Responden: %{customdata[2]}<br>"
                "Scope: %{customdata[3]}<extra></extra>"
            ),
        )
    )
    figure.update_layout(
...
def variable_score_chart(
    ...
    figure = go.Figure(
        go.Bar(
            ...
            hovertemplate=(
                "<b>%{x}</b><br>"
                "Skor rata-rata: %{y:.3f}<br>"
                "Interpretasi: %{customdata[0]}<br>"
                "Indikator dihitung: %{customdata[1]}<br>"
                "Scope: %{customdata[2]}<extra></extra>"
            ),
        )
    )
...
def render_filter_panel(
    ...
    with st.container(key="control_panel"):
        ...
        action_spacer, action_left, action_right = st.columns([4.4, 1, 1])
        with action_left:
            if st.button(
                "Reset Semua",
                key="panel_reset_all",
        ...
        with st.form("filter_control_form", border=False):
            survey_filter, review_filter, display_filter = st.columns(3)
            with survey_filter:
...
def active_filter_chips(
    ...
    if filters.get("questionnaire_categories"):
        chips.append(
            "Kategori indikator: "
...
def review_trend_chart(
    ...
    figure = go.Figure(
        go.Scatter(
            ...
            hovertemplate=(
                "<b>%{x}</b><br>%{y} dari %{customdata[0]} ulasan "
                "(%{customdata[1]:.1f}%)<br>"
                "Scope: %{customdata[2]}<extra></extra>"
            ),
        )
...
def render_data_explorer(
    ...
    if questionnaire is None or questionnaire.empty:
        st.info("Data hasil kuesioner tidak tersedia.")
    else:
        public_questionnaire = add_number_column(
            questionnaire[["label", "pertanyaan", "rata_rata"]].copy()
        )
        st.dataframe(
...
def render_output_and_presentation(
    ...
    if invariant_errors:
        st.error(
            "Validasi baseline menemukan masalah. Dashboard belum boleh dianggap "
    ...
    deliverables = [
        ...
        ("Screenshot", "Ambil tujuh tampilan utama setelah QA visual final."),
...
def render_conclusion(
    ...
    st.html(
        f"""
        <div class="insight-card">
            <h4>Ringkasan analitis &middot; {escape(scope_label)}</h4>
            <ul>
                ...
                <li>Kesimpulan ini bersifat deskriptif dan tidak menyatakan hubungan kausal.</li>
            </ul>
        </div>
        """
    )
