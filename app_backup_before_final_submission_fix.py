...
def render_top_header(last_refresh: datetime, loaded: bool) -> None:
    with st.container(key="top_header"):
        menu_column, brand_column, status_column, refresh_column = st.columns(
            [0.5, 4.6, 4.3, 0.6], vertical_alignment="center"
        )
        with menu_column:
            if st.button(
                "☰",
                key="hamburger_toggle_btn",
                help="Buka atau tutup Control Panel",
                width="stretch",
            ):
            ...
            st.html(
                ...
                <div class="brand-lockup">
                    ...
                    <div>
                        ...
                        <div class="brand-subtitle">Survey &amp; Review Analytics</div>
                    </div>
                </div>
                """
            )
        ...
        with refresh_column:
            if st.button(
                "↻",
                key="top_refresh_btn",
                help="Refresh Data dan clear cache",
                width="stretch",
            ):
                st.cache_data.clear()
                st.session_state.last_refresh = datetime.now(WIB)
    ...
    st.html(
        ...
        <section class="hero-section fade-in">
            <div class="hero-content">
                <div class="eyebrow">Fintech Experience Analytics</div>
                <h1 class="hero-title">DANA Insight Command Center</h1>
                <p class="hero-subtitle">
                    Dashboard interaktif untuk memahami pola penggunaan, skor pengalaman,
                    rating, dan sentimen pengguna aplikasi DANA.
                </p>
                <div class="badge-row">
                    <span class="hero-badge">Survey {survey_total:,} Responden</span>
                    <span class="hero-badge">Review {review_total:,} Ulasan</span>
                    <span class="hero-badge">Sentiment Analysis</span>
                    <span class="hero-badge">Interactive Dashboard</span>
                    <span class="hero-badge">Streamlit Cloud Ready</span>
                </div>
                <div class="hero-stat-row">
                    ...
                    <div>
                        ...
                        <span class="hero-stat-label">Skor Total</span>
                    </div>
                    <div>
                        <span class="hero-stat-value">{review_average:.2f} / 5</span>
                        <span class="hero-stat-label">Rating Total</span>
                    </div>
                    <div>
                        <span class="hero-stat-value">{positive_percentage:.1f}%</span>
                        <span class="hero-stat-label">Positif Total</span>
                    </div>
                </div>
            </div>
        </section>
    ...
    with right:
        with st.container(key="chart_overview_trend", border=True):
            ...
            if reviews is not None and not reviews.empty and date_column in reviews.columns:
                trend_source = reviews.dropna(subset=[date_column]).copy()
                if not trend_source.empty:
                    trend_source["hari"] = trend_source[date_column].dt.date
                    daily = (
                        trend_source.groupby("hari")
                        .size()
                        .reset_index(name="jumlah")
                        .sort_values("hari")
                    )
                    plot_chart(
                        line_chart(daily, "hari", "jumlah", "Tren Ulasan Harian"),
                        "overview_trend",
                    )
                    st.caption("Pergerakan volume ulasan pada rentang tanggal aktif.")
                else:
