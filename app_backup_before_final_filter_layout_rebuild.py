...
def main() -> None:
    ...
    with st.sidebar:
        if st.session_state.get("filter_open", False):
            ...
            filter_icon_html = (
                f'<img class="filter-rail-marker-icon" '
                f'src="data:image/png;base64,{img_to_base64(filter_icon)}" alt="" />'
                if filter_icon else ""
            )
            st.html(
                '<div class="filter-rail-marker">'
                + filter_icon_html
                + '<div class="filter-rail-marker-text">'
                '<strong>Filter &amp; Control</strong>'
                '<span>Ubah filter, lalu tekan <em>Terapkan Filter</em>.</span>'
                '</div></div>'
            )
